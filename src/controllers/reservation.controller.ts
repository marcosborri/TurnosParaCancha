...
export class ReservationController {
    ...
    getReservationById = async (req: Request, res: Response) => {
        ...
        try {
            const getReservationById =
                await this.reservationService.getReservation(Number(id))
    ...
    addNewReservation = async (req: Request, res: Response) => {
        ...
        const parseId = (v: any) => {
            ...
            if (v && typeof v === 'object')
                return Number(v.id ?? v._id ?? v.idField)
            return NaN
        }
        ...
        try {
            ...
            const searchSameReservation = allReservations.find((r: any) => {
                ...
                const sameStart =
                    rStart && rStart.getTime() === startDate.getTime()
            ...
            const saved = await this.reservationService.addReservation(
                reservation
            )
    ...
    eliminateReservation = async (req: Request, res: Response) => {
        ...
        try {
            await this.reservationService.deleteReservation(Number(id)) // <-- ACA EL ERROR
    ...
    startReservationEdit = async (req: Request, res: Response) => {
        ...
        try {
            const startEdited =
                await this.reservationService.editReservationStart({
                    id: Number(id),
    ...
    paidReservationEdit = async (req: Request, res: Response) => {
        ...
        try {
            const paidEdited =
                await this.reservationService.editReservationPaid({
                    id: Number(id),
                    paid,
                })
